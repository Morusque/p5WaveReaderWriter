...
class Sample16 extends Sample {
  public int MAX_VALUE = (int)Math.pow(2, 15);
  public short value;
...
{
  public int MAX_VALUE = (int)Math.pow(2, 23);
  public int value;
...
class Sample32 extends Sample {
  public int MAX_VALUE = (int)Math.pow(2, 31);  
  public int value;
...
class SamplingInfo {
  ...
  public class Instances {
    public SamplingInfo Default = new SamplingInfo(
      "Default", 
      16, // chunkSize
      (short)1, // formatCode
...
class WavFile {
  ...
  public WavFile (SamplingInfo samplingInfo, double[][] samplesForChannels) {
  ...
  public void setSamplesFromDouble(double[][] input) {
    ...
    samplesForChannels = buildManySamplesFromDoubles(samplingInfo, input);
  }
  ...
  public void writeToFilePath()
  {
    try
    {
      ...
      byte[] samplesAsBytes = convertManySamplesToBytes(this.samplesForChannels, this.samplingInfo);    
...
WavFile readWaveFromFilePath (String filePathToReadFrom) {
  ...
  try {
    ...
    Sample[][] samplesForChannels = buildManyFromBytes(samplingInfo, samplesForChannelsMixedAsBytes);
...
Sample[][] buildManySamplesFromDoubles (SamplingInfo samplingInfo, double[][] input) {
...
Sample[][] buildManyFromBytes (SamplingInfo samplingInfo, byte[] bytesToConvert) {
...
static Sample[] concatenateSets(Sample[][] setsToConcatenate) {
...
byte[] convertManySamplesToBytes(Sample[][] samplesToConvert, SamplingInfo samplingInfo) {
...
Sample[] superimposeSets(Sample[][] setsToSuperimpose) {
...
void saveWaveToFile(String url, float[][] samples) {
  double[][] samplesDouble = new double[samples.length][];
  for (int i=0; i<samples.length; i++) {
    samplesDouble[i] = new double[samples[i].length];
    for (int j=0; j<samples[i].length; j++) samplesDouble[i][j] = (double)samples[i][j];
  }
  saveWaveToFile(url, samplesDouble);
}

void saveWaveToFile(String url, double[][] samples) {
  SamplingInfo tempInfos = new SamplingInfo("wave", 16, (short)1, (short)samples.length, 44100, (short)16);
  WavFile tempFile = new WavFile(tempInfos, samples);
  tempFile.writeToFilePath(url);
}
